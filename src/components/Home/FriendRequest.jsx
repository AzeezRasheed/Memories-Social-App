import React from "react";
import tw from "twin.macro";
import styled from "styled-components";

const Container = styled.div`
  ${tw`   flex-col flex h-full pt-4     `}
`;

const TopHeader = styled.div`
  ${tw`flex justify-between w-full items-center  `}
`;

const Title = styled.h1`
  ${tw`w-72 font-bold text-gray-500 text-sm items-start`}
`;

const SeeAll = styled.li`
  ${tw`text-black flex w-20 hover:text-gray-500   font-bold text-xs list-none items-end `}
`;

const Followers = styled.div`
  ${tw`
  flex flex-col items-start gap-4 justify-center w-full
`}
`;

const Follower = styled.div`
  ${tw`flex flex-row justify-between items-center gap-2   w-full`}
`;

const Avatar = styled.div`
  ${tw`   h-full overflow-hidden rounded-full `}
`;
const RightRow = styled.div`
  ${tw`flex flex-col gap-2 flex-1 `}
`;

const Username = styled.h2`
  ${tw`text-sm font-medium `}
`;

const ButtonContainer = styled.div`
  ${tw`flex flex-row justify-between gap-2`}
`;
const Button = styled.button`
  ${tw`w-full py-2 rounded p-2 cursor-pointer items-center text-center`}
`;

const AcceptButton = styled(Button)`
  ${tw` bg-blue-400 hover:bg-white transition ease-in-out duration-75 font-bold text-sm`}
`;
const RejectButton = styled(Button)`
  ${tw`bg-white hover:bg-blue-400  transition ease-in-out duration-75 font-bold text-sm`}
`;
function FriendRequest() {
  return (
    <Container>
<div className="flex flex-col fixed md:w-[270px] lg:w-[260px] gap-4 w-full pl-2 ">
<TopHeader>
        <Title>Friend Requests</Title>
        <SeeAll>
          <a href="/" className="w-full">
            See All
          </a>
        </SeeAll>
      </TopHeader>

      <Followers>
        <Follower>
          <Avatar>
            <img
              src="https://png.pngtree.com/png-vector/20220709/ourmid/pngtree-businessman-user-avatar-wearing-suit-with-red-tie-png-image_5809521.png"
              alt="profile "
              className=" w-16 h-16  rounded-full "
            />
          </Avatar>
          <RightRow>
            <Username>Abdulazeez Abdulrasheed</Username>
            <ButtonContainer>
              <AcceptButton>
                <a href="/" className="w-full">
                  Accept
                </a>
              </AcceptButton>
              <RejectButton>
                <a href="/" className="w-full">
                  Reject
                </a>
              </RejectButton>
            </ButtonContainer>
          </RightRow>
        </Follower>

        <Follower>
          <Avatar>
            <img
              src="data:image/jpeg;base64,"
              alt="profile "
              className=" w-16 h-16  rounded-full "
            />
          </Avatar>

          <RightRow>
            <Username>Abdulazeez Abdulrasheed</Username>
            <ButtonContainer>
              <AcceptButton>
                <a href="/" className="w-full">
                  Accept
                </a>
              </AcceptButton>
              <RejectButton>
                <a href="/" className="w-full">
                  Reject
                </a>
              </RejectButton>
            </ButtonContainer>
          </RightRow>
        </Follower>
      </Followers>
</div>
    </Container>
  );
}

export default FriendRequest;
