import React from "react";
import tw from "twin.macro";
import styled from "styled-components";

const Container = styled.div`
  ${tw`   flex-col flex h-full pt-4 px-6 gap-6   `}
`;

const TopHeader = styled.div`
  ${tw`flex justify-between w-full items-center  `}
`;

const Title = styled.h1`
  ${tw`w-72 flex-1 font-bold text-gray-500 text-sm items-start`}
`;

const SeeAll = styled.li`
  ${tw`text-black flex w-20  hover:text-gray-500 text-end  font-bold text-xs list-none items-end `}
`;

const Followers = styled.div`
  ${tw`
  flex flex-col items-start gap-4 justify-center w-full
`}
`;

const Follower = styled.div`
  ${tw`flex justify-between items-center w-full`}
`;

const Avatar = styled.div`
  ${tw`   h-full overflow-hidden rounded-full `}
`;
const RightRow = styled.div`
  ${tw`flex flex-col gap-2 flex-1 `}
`;

const ButtonContainer = styled.div`
  ${tw`flex flex-row justify-between gap-2`}
`;
const Button = styled.button`
  ${tw`w-full py-2 rounded p-2 cursor-pointer items-center text-center`}
`;

const AcceptButton = styled(Button)`
  ${tw` bg-blue-400 hover:bg-white transition ease-in-out duration-75 font-bold text-sm`}
`;
const RejectButton = styled(Button)`
  ${tw`bg-white hover:bg-blue-400  transition ease-in-out duration-75 font-bold text-sm`}
`;

const BasicProfile = styled.div`
  ${tw`flex justify-between items-center w-full`}
`;

const ProfileImage = styled.div`
  ${tw` h-full overflow-hidden rounded-full`}
`;

const ProfileInfo = styled.div`
  ${tw` flex flex-col gap-1 text-start items-start `}
`;

const FullName = styled.div`
  ${tw` text-sm text-slate-600 `}
`;

const Username = styled.h2`
  ${tw`text-sm font-bold text-black `}
`;

const RightButton = styled.button`
  ${tw` text-[12px] text-blue-500  flex-1 font-bold`}
`;

function FriendsComponent() {
  return (
    <Container>
      <TopHeader>
        <Title>Friends</Title>
        <SeeAll>
          <a href="/" className="w-full">
            See All
          </a>
        </SeeAll>
      </TopHeader>
      <div className="flex flex-col   gap-4 w-full pl-2 ">
      

        <Followers>
          <Follower>
            <div className="flex flex-row gap-2 items-center w-full ">
              <Avatar>
                <img
                  src="https://png.pngtree.com/png-vector/20220709/ourmid/pngtree-businessman-user-avatar-wearing-suit-with-red-tie-png-image_5809521.png"
                  alt="profile "
                  className=" w-10 h-10  rounded-full "
                />
              </Avatar>
              <ProfileInfo>
                <Username>Rashie 10k</Username>
                <FullName>Abdulazeez Abdulrasheed</FullName>
              </ProfileInfo>
            </div>
            <RightButton>Chat</RightButton>
          </Follower>
        </Followers>

        <Followers>
          <Follower>
            <div className="flex flex-row gap-2 items-center w-full ">
              <Avatar>
                <img
                  src="data:image/jpeg;base64,"
                  alt="profile "
                  className=" w-10 h-10  rounded-full "
                />
              </Avatar>
              <ProfileInfo>
                <Username>Rashie 10k</Username>
                <FullName>Abdulazeez Abdulrasheed</FullName>
              </ProfileInfo>
            </div>
            <RightButton>Chat</RightButton>
          </Follower>
        </Followers>
      </div>
    </Container>
  );
}

export default FriendsComponent;
